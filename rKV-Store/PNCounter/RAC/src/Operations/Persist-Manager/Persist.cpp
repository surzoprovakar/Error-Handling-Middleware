...
extern "C"
{
    ...
    const char *undo(const char *id, int opt_nums)
    {
        ...
        if (opt_nums == -1)
        {
            if (stat_undo(3)) // 3 prev updates for example purpose, can be configured
            {
            ...
            {
                cout << "no undo required based on statistical" << endl;
                return NULL;
            }
        }
    ...
    {
        void *handle = dlopen("src/Operations/Plug-in/statistics.so", RTLD_NOW);
